...
begin
  inherited;
  if MessageDlg('Deseja excluir a venda selecionada?', mtConfirmation, [mbYes, mbNo], mrNo) = mrNo then
    Exit
  else
  begin
    ExcluirVendas();
    MessageDlg('Venda excluida com sucesso!', mtInformation, [mbOK],0);
    VerificaBotoes(opInicio);
  end;
end;
...
begin
  ...
  try
    ...
    if Venda.Excluir(StrToInt(EdtCodVenda.Text), sErro) = False then
      raise Exception.Create(sErro);
